...
from settings import (
    W, H, GROUND, FPS,
    DAY_BG, DAY_GROUND, DAY_DINO, DAY_CACTUS, DAY_CLOUD,
    NIGHT_BG, NIGHT_GROUND, NIGHT_DINO, NIGHT_CACTUS, NIGHT_CLOUD,
    BG_COLOR, GROUND_COLOR, DINO_COLOR, CACTUS_COLOR, CLOUD_COLOR,
    START_SPEED, MAX_SPEED, RECORD_ANN_COLOR,
)
from star   import Star
from cloud  import Cloud
from cactus import AdvancedCactus
...
def update_game_colors(current_score):
    ...
    if is_night:
        target_bg, target_ground = NIGHT_BG, NIGHT_GROUND
        target_dino, target_cactus, target_cloud = NIGHT_DINO, NIGHT_CACTUS, NIGHT_CLOUD
        cfg.HI_SCORE_COLOR = (180, 180, 180)
        night_alpha += (1.0 - night_alpha) * 0.02
    else:
        target_bg, target_ground = DAY_BG, DAY_GROUND
        target_dino, target_cactus, target_cloud = DAY_DINO, DAY_CACTUS, DAY_CLOUD
        cfg.HI_SCORE_COLOR = (150, 150, 150)
...
def reset_game():
    global score, game_speed, obstacles, clouds, spawn_timer, game_over, new_high_score_reached, night_alpha
    score                 = 0.0
...
while True:
    ...
    if not game_started:
        ...
        hi_score_surf    = font.render(f"HI: {high_score}",      True, cfg.HI_SCORE_COLOR)
        ...
        if spawn_timer > 75:
            if random.random() < 0.85:
                obstacles.append(AdvancedCactus(W + 20))
                rand_group = random.random()
                if rand_group < 0.4:
                    obstacles.append(AdvancedCactus(W + random.randint(55, 75)))
                    if rand_group < 0.15:
                        obstacles.append(AdvancedCactus(W + random.randint(110, 135)))
            spawn_timer = 0
        ...
        hi_text  = font.render(f"HI {high_score:05d}", True, cfg.HI_SCORE_COLOR)
